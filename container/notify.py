...
def main():
    ...
    flag_file = "/home/user/.claude/notify-on"
    ...
    events = {
        "stop": ("SwarmFleet — Task Complete", "Claude has finished the current task.", "info"),
        "error": ("SwarmFleet — Something Went Wrong", "A tool use failure occurred. Check the session for details.", "warning"),
    }
    title, body, notify_type = events.get(event, (
        "SwarmFleet — Notification",
        f"Event: {event}",
        "info",
    ))
