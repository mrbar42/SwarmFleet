...
export function createContentPreview(
  ...
  if (!content || content.trim().length === 0) {
    return { preview: "", hasMore: false, totalLines: 0, previewLines: 0 };
  }
  ...
  if (totalLines <= maxPreviewLines) {
    return { preview: content, hasMore: false, totalLines, previewLines: totalLines };
  }
...
export function createEditResult(
  ...
  if (!isValidStructuredPatch(structuredPatch)) {
    return { details: fallbackContent, summary: "", defaultExpanded: true };
  }
